## Run from paintball:sniper_start_raycast
...
execute as @e[tag=!Dead,tag=!raycasting,type=!#paintball:not_mob,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function paintball:sniper_collide

scoreboard players add .distance sniper_rc 1
...
execute if score .distance sniper_rc matches ..1800 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #paintball:raycast_pass run function paintball:sniper_raycast
